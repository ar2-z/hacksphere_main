...
const participantLinks = [
  ...
  { to: '/teams', label: 'My Team', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z' },
...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-deep-black flex">
      <aside
        className={`${collapsed ? 'w-16' : 'w-60'} h-screen sticky top-0 bg-midnight/60 border-r border-midnight-lighter/30 flex flex-col transition-all duration-300`}
      >
        <div className={`h-14 flex items-center ${collapsed ? 'justify-center' : 'px-4'} border-b border-midnight-lighter/30`}>
          {collapsed ? (
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 bg-cyan/10 border border-cyan/30 rounded-lg flex items-center justify-center shrink-0">
                <svg className="w-4 h-4 text-cyan" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
              </div>
              <span className="text-sm font-semibold text-white tracking-tight" style={{ fontFamily: 'var(--font-display)' }}>
                HackSphere
              </span>
            </div>
          )}
        ...
        <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-0.5">
          {links.map((link) => (
            ...
            >
              <svg className={`w-4 h-4 shrink-0 ${collapsed ? '' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className={`p-3 border-t border-midnight-lighter/30 ${collapsed ? 'flex flex-col items-center gap-2' : ''}`}>
          <button
            onClick={() => setCollapsed(!collapsed)}
            className="w-full flex items-center justify-center py-2 text-silver-dim hover:text-silver rounded-lg hover:bg-midnight-light/30 transition-all cursor-pointer"
          >
          ...
          {!collapsed && (
            <button
              ...
              className="w-full flex items-center gap-3 px-3 py-2.5 text-[13px] text-silver-dim hover:text-red-400 hover:bg-red-400/5 rounded-lg transition-all cursor-pointer"
            >
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        <header className="h-14 border-b border-midnight-lighter/30 bg-midnight/30 backdrop-blur-xl flex items-center justify-between px-6 sticky top-0 z-10">
          <div />
          <div className="flex items-center gap-3">
            ...
            <div className="w-8 h-8 bg-midnight-light/60 border border-midnight-lighter/60 rounded-full flex items-center justify-center">
              <span className="text-sm font-medium text-cyan/70">{user?.full_name?.charAt(0)?.toUpperCase()}</span>
            </div>
          </div>
        </header>
